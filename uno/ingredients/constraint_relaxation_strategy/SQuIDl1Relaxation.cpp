...
#include "ingredients/globalization_strategy/l1MeritFunction.hpp"
...
double SQuIDl1Relaxation::l(const Direction& direction, const Iterate& current_iterate) const {
   const auto linearized_constraint = current_iterate.evaluations.constraints + current_iterate.evaluations.constraint_jacobian * direction.primals;
...
double SQuIDl1Relaxation::Ropt(Iterate& current_iterate, double objective_multiplier, const Multipliers& multipliers) const {
   // stationarity error
   this->evaluate_lagrangian_gradient(current_iterate, multipliers);
   const auto scaled_lagrangian = objective_multiplier * current_iterate.lagrangian_gradient.objective_contribution +
                                 current_iterate.lagrangian_gradient.constraints_contribution;
...
double SQuIDl1Relaxation::Rinf(Iterate& current_iterate, const Multipliers& multipliers) const {
   // stationarity error
   this->evaluate_lagrangian_gradient(current_iterate, multipliers);
   const auto scaled_lagrangian = current_iterate.lagrangian_gradient.constraints_contribution;
   double error = norm_inf(scaled_lagrangian);
...
double SQuIDl1Relaxation::compute_w(const Direction& feasibility_direction, const Direction& optimality_direction, const Iterate& current_iterate) {
   ...
   while (not termination) {
      ...
      const auto trial_direction = view(weight * feasibility_direction.primals + (1 - weight) * optimality_direction.primals, 0, this->model.number_variables);
      // update reduction in linearized feasibility model
      const auto trial_linearized_constraints = current_iterate.evaluations.constraints + current_iterate.evaluations.constraint_jacobian * trial_direction;
      ...
      if (delta_l_d >= this->parameters.beta * delta_l_dbar) {
         ...
         if (weight == 0. || upper_bound - lower_bound <= 1e-8) {
            ...
            DEBUG2 << "Interpolated direction:  "; print_vector(DEBUG2, trial_direction);
         }
         else {
            // keep the first half
            upper_bound = weight;
...
void SQuIDl1Relaxation::compute_feasible_direction(Statistics& statistics, Iterate& current_iterate, Direction& direction,
   ...
   if (this->tolerance < this->v(current_iterate) &&
      ...
      const double multipliers_distance = norm_2(current_iterate.multipliers.constraints + (-1)*current_iterate.feasibility_multipliers.constraints);
